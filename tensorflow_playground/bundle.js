(function() {
    ...
    2: [ function(require, module, exports) {
        ...
        function classifyTwoGaussData(numSamples, noise) {
            var points = [];
            var varianceScale = d3.scale.linear().domain([ 0, .5 ]).range([ .5, 4 ]);
        ...
        function regressGaussian(numSamples, noise) {
            var points = [];
            var labelScale = d3.scale.linear().domain([ 0, 2 ]).range([ 1, 0 ]).clamp(true);
            var gaussians = [ [ -4, 2.5, 1 ], [ 0, 2.5, -1 ], [ 4, 2.5, 1 ], [ -4, -2.5, -1 ], [ 0, -2.5, 1 ], [ 4, -2.5, -1 ] ];
            function getLabel(x, y) {
                var label = 0;
                gaussians.forEach(function(_a) {
                    var cx = _a[0], cy = _a[1], sign = _a[2];
                    var newLabel = sign * labelScale(dist({
                        x: x,
                        y: y
                    }, {
                        x: cx,
        ...
        function classifyCircleData(numSamples, noise) {
            ...
            function getCircleLabel(p, center) {
                return dist(p, center) < radius * .5 ? 1 : -1;
            }
            for (var i = 0; i < numSamples / 2; i++) {
                var r = randUniform(0, radius * .5);
            ...
            for (var i = 0; i < numSamples / 2; i++) {
                var r = randUniform(radius * .7, radius);
                var angle = randUniform(0, 2 * Math.PI);
                var x = r * Math.sin(angle);
                var y = r * Math.cos(angle);
                var noiseX = randUniform(-radius, radius) * noise;
    ...
    3: [ function(require, module, exports) {
        ...
        var HeatMap = function() {
            function HeatMap(width, numSamples, xDomain, yDomain, container, userSettings) {
                ...
                var tmpScale = d3.scale.linear().domain([ 0, .5, 1 ]).range([ "#f59322", "#e8eaeb", "#0877bd" ]).clamp(true);
                ...
                this.canvas = container.append("canvas").attr("width", numSamples).attr("height", numSamples).style("width", width - 2 * padding + "px").style("height", height - 2 * padding + "px").style("position", "absolute").style("top", padding + "px").style("left", padding + "px");
        ...
        function reduceMatrix(matrix, factor) {
            ...
            if (matrix.length % factor !== 0) {
                throw new Error("The width/height of the matrix must be divisible by " + "the reduction factor");
            }
    ...
    4: [ function(require, module, exports) {
        ...
        var AppendingLineChart = function() {
            function AppendingLineChart(container, lineColors) {
                ...
                this.svg = container.append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    ...
    6: [ function(require, module, exports) {
        ...
        d3.select(".more button").on("click", function() {
            var position = 800;
            d3.transition().duration(1e3).tween("scroll", scrollTween(position));
        });
        ...
        function makeGUI() {
            ...
            var xAxis = d3.svg.axis().scale(x).orient("bottom").tickValues([ -1, 0, 1 ]).tickFormat(d3.format("d"));
            ...
            if (isInput) {
                ...
                if (/[_^]/.test(label)) {
                    ...
                    while ((myArray = myRe.exec(label)) != null) {
                        ...
                        text.append("tspan").attr("baseline-shift", sep === "_" ? "sub" : "super").style("font-size", "9px").text(suffix);
                    }
        ...
        function drawNetwork(network) {
            ...
            var layerScale = d3.scale.ordinal().domain(d3.range(1, numLayers - 1)).rangePoints([ featureWidth, width - RECT_SIZE ], .7);
            ...
            for (var layerIdx = 1; layerIdx < numLayers - 1; layerIdx++) {
                ...
                for (var i = 0; i < numNodes; i++) {
                    ...
                    for (var j = 0; j < node_1.inputLinks.length; j++) {
                        ...
                        if (targetIdWithCallout == null && i === numNodes_1 - 1 && link.source.id === lastNodePrevLayer.id && (link.source.id !== idWithCallout || numLayers <= 5) && link.dest.id !== idWithCallout && prevLayer.length >= numNodes_1) {
                            var midPoint = path.getPointAtLength(path.getTotalLength() * .7);
        ...
        function addPlusMinusControl(x, layerIdx) {
            var div = d3.select("#network").append("div").classed("plus-minus-neurons", true).style("left", x - 10 + "px");
            var i = layerIdx - 1;
        ...
        function updateUI(firstStep) {
            ...
            var selectedId = selectedNodeId != null ? selectedNodeId : nn.getOutputNode(network).id;
        ...
        function reset(onStartup) {
            ...
            var outputActivation = state.problem === state_1.Problem.REGRESSION ? nn.Activations.LINEAR : nn.Activations.TANH;
        ...
        function hideControls() {
            ...
            HIDABLE_CONTROLS.forEach(function(_a) {
                ...
                var label = hideControls.append("label").attr("class", "mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect");
        ...
        function generateData(firstTime) {
            ...
            var generator = state.problem === state_1.Problem.CLASSIFICATION ? state.dataset : state.regDataset;
