...
# 1 Line followers and edge followers
...
## 1.1 A simple line follower using two sensors
...
## 1.2 Writing a line-follower program
...
## 1.3 Creating a line-follower program
...
<font color='red'>JD: This seems like quite a leap for students. Perhaps refer them back to the code for moving the robot forwards/turning/logic? Or perhaps build it up more slowly, maybe with psuedo-code. Maybe a hint that you could use a while loop that causes the robot to stop when neither sensor sees black?</font>
...
If both sensors see black then the robot stops, which may not be the desired behaviour!

Also, the robot falls off the line quite easily if the speed is set too high.

<font color='red'>JD: Do we really need to cast to use floating-point division? It looks like Python 3.7 is being used I get floating-point division without needing to cast...</font>
<!-- #endregion -->
...
## 1.4 A simple edge follower using a single sensor
