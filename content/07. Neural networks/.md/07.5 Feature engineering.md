...
signature = generate_signature(test_image, fill=0, show=True, threshold=127)
```
...
*This may take some time — the signature generating code is far from optimal! — so for now only generate signatures for some of the sample images.*
<!-- #endregion -->
...
<!-- #region tags=["alert-warning"] -->
*To save time creating the signatures lists, I have saved a copy of it using the Python `pickle()` function:*
...
*You can load the pre-calculated signatures in with commands of the form:*
