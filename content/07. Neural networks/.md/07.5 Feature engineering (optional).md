...
Let’s just recap what happened when we trained an MLP on the original dataset.
...
The signature comprises sets of four values, one set per row of the image, image dataframe, or image array:
...
Before the signature is generated, the image is converted to a binarised black and white image. The threshold used to set the black and white threshold value is pass by passing a threshold parameter, which defaults at a value of 127 (`threshold=127`).<span style="color:red">JD: previous sentence doesn't seem to make sense, especially 'is pass by passing'.</span>
...
*How does it compare to the behaviour of the network trained on the full-size image data?*
<!-- #endregion -->
