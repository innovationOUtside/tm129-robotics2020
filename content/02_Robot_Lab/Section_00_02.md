...
<!-- #region tags=["figure-description"] -->
Figure description: a flowchart diagram that describesthe behaviour of the *detecting black and grey* program.
...
It is sometimes useful to have just a single branch to the `if` statement. Python provides a simple `if...` statement for this purpose, which you may recall from a previous notebook.
...
For example, if we have a value of `sensor_value = 86` exactly, the conditional test `sensor_value > 86` will evaluate as `False` because the variable value 86 is not strictly greater than threshold value 86. But if the sensor value is `sensor_value = 86.00000000000000000001`, the test will evalute as `True`.
<!-- #endregion -->
...
## Noise in Real and Simulated Robots
