...
cond(no)->e
```
<font color='red'>JD: Originally, in the flowchart above, the 'Is the count less than 20' decision had 'no' pointing back to 'Swim a length' (and 'yes' to end), which is the wrong way round. I've attempted to amend the flowchart so that 'Is the count less than 20' has 'yes' returning to 'Swim a length' (and 'no' to the end). Please check I've done this correctly.</font>
...
%load_ext nbtutor
```
<font color='red'>JD: I get the message: "The nbtutor extension is already loaded. To reload it, use: %reload_ext nbtutor"</font>
...
<!-- #endregion -->
<font color='red'>JD: to be resolved/hidden from students?</font>
...
<font color='red'>JD: as noted in week 1 notebook, the nbtutor extension isn't working correctly for me (I see no buttons, just the drop-down list).</font>
...
<font color='red'>JD: Given that f-strings can't be used in the simulator and I'm not sure they are even used in TM112, I think it might be simpler here to give the hint that you can print several variables and strings at once by using commas in the print statement (e.g. "print('Today is', day_of_week)" )</font>
<!-- #endregion -->
...
```
<font color='red'>JD: Given that students are not expected to have done any programming before, I think some might need more help here. Ww need to say something about indentation in Python. And explain what += means. They might wonder why the list is split over two lines...</font>
...
<font color='red'>JD: as noted previously, for me the angles are wrong (it looks like three sides of an octogon).</font>
...
print('All done...')
```
<font color='red'>JD: Add some commentary here about what students *should* have seen?</font>
...
print('All done...')
```
<font color='red'>JD: nbtutor suddenly started working here, but looks very complicated with 19 variables, 16 lists/dicts/tuples/instances, and dozens of arrows (using the 'Memory' drop-down list).</font>
...
<font color='red'>JD: I can't see why, but the following isn't coming out as a code cell as expected.</font>
...
The Python `random` functon from the `random` package is capabale of generating a random number between 0 and 1, as you will see if you run the following cell repeatedly. (The package is only imported once despite multiple calls to it.)<font color='red'>JD: Perhaps this should be be 'The package only needs to be imported once...' because there aren't (yet) multiple calls to it.</font>
<!-- #endregion -->
...
<font color='red'>JD: Perhaps add a hint that >= is used to test whether what is on the left of >= is greater than or equal to what is on the right of it (for students new to programming)?</font>
...
```
<font color='red'>JD: Perhaps explain why you've used `counter = counter + 1` here instead of the equivalent `counter += 1` ?</font>
...
<font color='red'>JD: It doesn't! It carries on until it meets the BLACK line as that is the first time the sensor value falls below 99. The variable `sensor_value` is using light values from 255 (white) to (0) black, whereas I think your code is expecting it to use 100 (white) to 0 (black).</font>
...
The program works by checking the value from one of the robot’s sensors: a downward-facing light sensor, which you will meet in more detail in a later notebook. The sensor returns a ‘reflected light’ reading: a percentage value which relates to the colour of the background over which the robot is travelling. The simulator output display window shows the sensor value, starting at `100` <font color='red'>JD: 255 for me </font>when the robot is on the plain white background. This value is above the conditionally tested threshold value of `99` used in the original program’s `while` statement, and so the program continues looping round the while loop. When the robot encounters the first grey line, the sensor returns a lower value of just over 87 when I ran the program.<font color='red'>JD: 86.27 according to the simulation screen, but 220 is the value of `sensor_value`.</font>
...
The following program, for example, when downloaded to the simulator, will cause the robot (?!)<font color='red'>JD: what does (?!) mean? Perhaps the name for the robot? Given that in the following program the voice sounds female, perhaps it should be called 'Simone'? </font> to count aloud.
...
```
<font color='red'>JD: I get the error message: "ModuleNotFoundError: No module named 'pyttsx3'" so the next code cell won't work either.</font>
