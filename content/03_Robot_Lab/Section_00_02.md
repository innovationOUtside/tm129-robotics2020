...
%load_ext nbtutor
```
<font color='red'>JD: To be moved into the first notebook.</font>
...
```
<font color='red'>JD: It seems odd that this notebook has no 'Activities' (except for the optional one). Add some 'Activity' headings in this notebook?</font>
...
## 1.1 Using functions in robot control programs
...
Tweak the parameter settings until the robot approximately traces out the shape of a square.<font color='red'>JD: First use of the word 'parameter' (we've used 'argument' so far. Perhaps explain?</font>

<font color='red'>JD: In the following, the comment says to run for 1 rotation, but the value used is set to 4. As before, this draws the first four sides of an octagon for me...</font>
...
<font color='red'>JD: In the following, the comment says to run for 1 rotation, but the value used is set to `sides`.</font>
