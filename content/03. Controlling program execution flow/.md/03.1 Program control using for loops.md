...
Load in the simulator and enabled the simulator magic:
<!-- #endregion -->
...
The conditional `if` statement can be used within a  `for` loop to interrupt the control flow within the loop either by foreshortening code execution with in the loop (`continue`) or breaking out of the loop (`break`):
