...
print('All done...')
```
As you do, you should see the `item` variable updated to each item `one`, `two`, `three`, `fish` and `four` in turn. As the program iterates through the loop, the current value of the `item` variable is displayed. The value of the `item` variable is checked, and if it *does not* equal `three` the loop continues by displaying the word `and`, giving a message that builds up as *one and two and three*.
