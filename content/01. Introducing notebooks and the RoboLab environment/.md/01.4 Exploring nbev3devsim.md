...
<!-- #region tags=["alert-success"] -->
Block cell magics *must* be prefixed by two percentage signs, — `%%` _ and *must* appear at the very start of the first line of a code cell. Even one or more blank lines or comment lines at the start of a code cell before the `%%` cell magic prefix will break things.
