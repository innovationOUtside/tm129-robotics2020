...
tank_turn.on_for_rotations(-100, SpeedPercent(40), 0.826)
```
<font color='red'>JD: This draws what looks like the first four sides of an octagon (I needed a value of about 1.7 to get something like a square).</font>
...
You have already seen how you could use a loop to simplify the square drawing program<font color='red'>JD: no we haven't!</font>, but you would still be having to delve into the depths of the program to change the values. And the values would still be *literal* values. So how can we improve things?
...
<font color='red'>JD: Note that in the following, the comments still refer to '1 rotation' even though a constant STRAIGHT_ROTATIONS (which is still currently 1) is now being used...</font>
...
<font color='red'>JD: there's inconsistency with heading levels used for activities (this activity has a second-level heading, whereas the previous activity has a third-level heading).</font>
...
<font color='red'>JD: Note that in the following, the comments still refer to '1 rotation' even though a constant STRAIGHT_ROTATIONS is now being used and is currently set to 6, so the comment is wrong.</font>
...
If you have not already done so, try adjusting the values of `forwardTime` <font color='red'>JD: What is `forwardTime`? I can't see it in the code. </font>so that the robot goes as close as possible to the grey square without touching it. Don’t spend too long on this.
...
You have already seen how variables can change their value as a program executes using the `nbtutor` extension. <font color='red'>JD: Where? (Certainly not in Week 1.)</font>
...
%load_ext nbtutor
```
<font color='red'>JD: I get told: The nbtutor extension is already loaded. To reload it, use:   %reload_ext nbtutor</font>
...
Run the code cell, then step through each line of code one line at a time using the `nbtutor` *Next >* button.<font color='red'>JD: I've not been able to test this as the nbtutor buttons don't appear for me (even after I've enabled the nbtutor extension I only get a grey bar above the cell with just a drop-down list containing '-', 'Memory' and 'Timeline' - so no buttons).</font>
...
Load the *Grey\_bands* background in to the simulator, and download and run the following program, which you may recall from the previous notebook <font color='red'>JD: No, this code looks completely new (it's certainly not in the 'previous' notebook).</font>. Observe the values that are displayed in the simulator output window.
