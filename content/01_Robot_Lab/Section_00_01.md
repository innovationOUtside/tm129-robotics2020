...
## Some basics of computer programming
...
The *‘stop the motors at the end of the program’* behaviour is a specific feature of the simulator we are using. This behaviour is not guaranteed in other environments that can be used to program either simulated or real robots. To be safe, it’s often worth making sure you turn off the motors at the end of a program so you know for sure what state they are in when the program terminates.
<!-- #endregion -->
...
This section provides a quick overview of what happens when you use a notebook to program the simulated robot in terms of managing computer resources.
...
When you run the simulated robot environment, the physical computer computer that runs the simulator is being used to do several different things. In partciular, it is used:
...
To preview the actual code downloaded to the simulator, click the simulator *Show Code* button.
...
The line number specified relates to the line number of the program actually run by the simulated robot. Some of the IPython magic commands used to configure code cells load in additional "boilerplate" statements in addition to the code explicitly contained in the code cell, so the line numbers in the error message will not correspond directly to the line number of the offending code line in the notebook code cell used to program the robot.

You can see the full program listing *as loaded into the simulated robot* from the *Show Code* option in the simulator.


### Passing code into the simulator

To write program code that can be passed to the simulator and executed within it to control the simulated robot, we add code to a code cell with some IPython magic in the first line:
...
# Wait here for 1 second, while motors continue running...
time.sleep(1)
...
%%sim_magic -b Empty_Map -x 800 -y 500
from ev3dev2.motor import MoveTank, SpeedPercent, OUTPUT_B, OUTPUT_C
...
*Run the following code cell to download the program to the simulator and then run the program in the simulator. Experiment with various settings for the motor speeds to get a feel for how  the relative left and right motor speeds determine the robots behviour.*
<!-- #endregion -->
...
## Writing programs for use in the RoboLab Simulator
...
If you run the following program in the simulator, then you will find that it drives the robot forward a short way and then turns on the spot for more than a right angle (ninety degrees, or a quarter of one complete rotation).

*The `-x`, `-y` and `-a` switches in the magic used to download the program to the simulator specifiy the original location and orientation of the robot in the simulator. The `-p` swtich enables pen down mode, to leave a trace of where the robot has moved, and the `-C` switch clears any previously made traces.*

```python
%%sim_magic_preloaded -x 200 -y 500 -a 0 -p -C
...
tank_turn.on_for_rotations(100, -100, 2)
```
...
# Set the robot to turn on the spot
# and run for 1 rotation *of the wheels*
tank_turn.on_for_rotations(-100, SpeedPercent(75), 1.6)

```

<!-- #region activity=true hidden=true -->
With these settings, on my computer, I found I could turn the robot through about ninety degrees by setting the number of wheel rotations on the turn step to between 1.5 and 1.7.
<!-- #endregion -->
...
<!-- #region activity=true -->
### Activity — Uncommenting a line of code
...
<!-- #region activity=true -->
### Activity — Commenting out a line of code
...
<!-- #region activity=true -->
### Acitivity — Uncommenting an invalid line of code
...
<!-- #region tags=["alert-success"] -->
Professional developers use a wide range of tools to help them debug their programmes as they create them. These include variable inspectors and breakpoint tools that allow the program execution to be stopped and inspected at various points. Using these tools is beyond the scope of this module.
<!-- #endregion -->
...
### Downloading Programs to a Real Robot

Many, if not all, of the programs created to run on the simulated robot should also run on a real Lego EV3 robot running the ev3dev operating system. [This note](https://github.com/ev3dev/vscode-ev3dev-browser) may provide some clues about how to actually do this...
