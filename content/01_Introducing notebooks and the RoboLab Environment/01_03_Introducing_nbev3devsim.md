...
### Background — ev3dev` and the original `ev3devsim` simulator
...
## 3.3 How the simulator relates to the notebooks
...
*For more technical details (optional), see the `Appendix-nbev3devsim_notebook_architecture` notebook.*
...
### 3.3.1 Activity — Load the `nbev3devsim` simulator into the notebook environment
...
<!-- #region activity=true -->
### 3.3.2 Activity — Downloading code to the simulator and running it there
...
### 3.3.3 Configuring the simulator layout
...
A reference guide to the `nbev3devsim` user interface is also provided in the *Appendix-nbev3devsim_user_interface* notebook.
 


### 3.3.4 Creating and saving your own robot program code

The activities have been created within Jupyter notebooks using the embedded simulator widget. In many activities, you will have the opportunity to create and run your own programmes to run within the simulator, as well as within the notebook's own Python coding environment.
